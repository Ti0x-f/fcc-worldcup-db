...
echo "$($PSQL "select teams.name from games left join teams on games.winner_id = teams.team_id or games.opponent_id = teams.team_id where games.year=2014 and round='Eighth-Final' order by teams.name")"
